...
Usage:
    $ python path/to/train.py --data coco128.yaml --img 640
"""
...
from _utils.torch_utils import ModelEMA, select_device, intersect_dicts, torch_distributed_zero_first, de_parallel
from _utils.plots import plot_labels, plot_evolution
from _utils.loss import ComputeLoss
from _utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    check_requirements, print_mutation, set_logging, one_cycle, colorstr
from _utils.datasets import create_dataloader
from _utils.autoanchor import check_anchors
from fake_loader import attempt_create, attempt_load
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        model = attempt_create(pretrained=False).to(device)  # create
        ...
        model = attempt_create(pretrained=True).to(device)  # create
            ...
            if is_coco:  # COCO dataset
                for m in [last, best] if best.exists() else [last]:  # speed, mAP tests
                    results, _, _ = val.run(data_dict,
                                            batch_size=batch_size // WORLD_SIZE * 2,
                                            imgsz=imgsz,
                                            model=attempt_load(weights=m, device=device).half(),
