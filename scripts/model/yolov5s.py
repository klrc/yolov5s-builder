...
class Xyolov5s(nn.Module):
    def __init__(self, nc=80, pretrained=True):
        ...
        if pretrained:
            proto_path = 'prototype/yolov5s.pt'
            print(f'loading pretrained weight from {proto_path}')
            self.load_state_dict(torch.load(proto_path))
    ...
    def as_bilinear(self):
        self.up_p1 = nn.Upsample(None, 2, 'bilinear')
        self.up_p2 = nn.Upsample(None, 2, 'bilinear')
        return self
